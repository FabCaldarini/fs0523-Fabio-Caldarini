...
function dice() {
  x = Math.floor(Math.random(1) * 6) + 1;
  return x;
}
...
function onlyInLastMillennium(array) {
  return array.filter((movie) => {
    return movie.Year < 1999 && movie.Year > 999;
  });
}
...
function selectTD() {
  let tabled = document.getElementsByTagName("td");
  return tabled;
}
...
function print() {
  printed = document.getElementsByTagName("td");

  for (let i = 0; i < printed.length; i++) {
    console.log(printed[i].innerText);
  }
}
